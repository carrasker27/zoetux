...
<body background="lightBack4.jpg">

<?
	$id = $_GET['id'];
	$sel = "SELECT * FROM refacciones WHERE id = '$id'";
	conecta();
	$rol = mysql_query($sel);
	desconecta();
    ...
    <form id="form1">
  		<fieldset>      	
            ...
            <div class="field">
				...
				<label>
                	<? 
						$tri = "SELECT * FROM historial WHERE no_pieza = '$row[no_pieza]'";
						conecta();
						$tre = mysql_query($tri);
						desconecta();
						
						while($tro = mysql_fetch_assoc($tre))
						{
							
							if($tro['tipo'] == 'ALTA')
							{
								echo $tro['tipo']." de pieza el ".$tro['fecha']." a las ".$tro['hora']." por ".$tro['usuario_sdr']."<br><br>";
							}else
							{
								echo $tro['tipo']." de ".$tro['cantidad']." piezas el ".$tro['fecha']." a las ".$tro['hora']." por ".$tro['usuario_sdr']."<br><br>";
							}
						}
					?>
                </label>
			...
			<div class="field">
				<center><input type="button" value="Imprimir" class="submit-button" onclick="window.print();"/></center>
        	</div>
